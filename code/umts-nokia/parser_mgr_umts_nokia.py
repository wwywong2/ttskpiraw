...
def worker():
	...
	if proc_mode != 'cluster': # client - support multi master (zookeeper)
		exec_str = "/opt/spark/bin/spark-submit --master --master mesos://zk://mesos_master_01:2181,mesos_master_02:2181,mesos_master_03:2181/mesos --driver-memory 512m --executor-memory 512m --total-executor-cores 2 %s/kpi_parser_umts_nokia.py \"%s\" /mnt/nfs/ttskpiraw/input/umts-nokia/nokia_%03d \"imnosrf@mesos_fs_01|%s\" \"client\" &" % (curr_py_dir, jobname, filenum, output_dir)
	else: # cluster - currently not support multi master (zookeeper)
		exec_str = "/opt/spark/bin/spark-submit --master mesos://mesos_master_01:7077 --deploy-mode cluster --driver-memory 512m --executor-memory 512m --total-executor-cores 2 --py-files \"file:///home/imnosrf/ttskpiraw/code/umts-nokia/util.py,file:///home/imnosrf/ttskpiraw/code/umts-nokia/xmlparser_umts_nokia.py,file:///home/imnosrf/ttskpiraw/code/umts-nokia/config.ini\" %s/kpi_parser_umts_nokia.py \"%s\" /mnt/nfs/ttskpiraw/input/umts-nokia/nokia_%03d \"imnosrf@mesos_fs_01\|%s\" \"cluster\"" % (curr_py_dir, jobname, filenum, output_dir)
