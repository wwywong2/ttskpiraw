...
from xmlparser_umts_nokia import nokiaXmlParser
import util
...
def main(sc,inSeqFile,outfilename):
   ...
   try:
      ...
      if proc_mode == 'client':
         ...
         # add py reference
         #util.logMessage("addPyFile: %s" % curr_py_dir+'/xmlparser_umts_nokia.py')
         sc.addPyFile(curr_py_dir+'/xmlparser_umts_nokia.py')
      ...
      #textRDD = sc.wholeTextFiles(inSeqFile)
   ...
   util.logMessage("start writing file: %s" % output_filename.split('.txt')[0] + "_*.txt", extraLogFilePt)

   uuidstr = "NA"
   if len(redRDD) <= 0:
      util.logMessage("No data to write to file: %s" % output_filename.split('.txt')[0] + "_*.txt", extraLogFilePt)
   else:
      ...
      if (len(err) == 0):
         ...
         if len(value['err']) > 0: # have error
            ...
            if len(value['err']) > 0: # regular error
               newfilename = output_filename.split('.txt')[0] + "_umts_" + key + ".error.log"
            ...
            if len(value['err']) > 1: # nothing use error
               newfilename2 = output_filename.split('.txt')[0] + "_umts_" + key + ".errorNothingUse.log"
         ...
         # print detail         
         newfilename = output_filename.split('.txt')[0] + "_umts_" + key + ".txt"
         ...
         if len(value['data'][1]) > 0: # have summary lines
            # get summary header
            header2, err2 = nokiaXmlParser.GetHeaderIniSUM(curr_py_dir+'/config.ini', key)
            newfilename = output_filename.split('.txt')[0] + "_umts_" + key + "_sum.txt"
            if (len(err2) == 0):
   ...
   util.logMessage("finish writing file: %s" % output_filename.split('.txt')[0] + "_*.txt", extraLogFilePt)
