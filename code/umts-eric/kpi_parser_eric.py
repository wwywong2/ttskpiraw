...
def f_map(filetuple):
   ...
   try:
      ...
      #util.logMessage("%s: inside map func, after XMLParser(), before xml.ProcessXML()" % socket.gethostname())
      return xml.ProcessXML()
   except Exception as e:
      util.logMessage("%s: err in file: %s" % (socket.gethostname(), fn))
      return ""

def f_reduce(a, b):
   #util.logMessage("%s: inside reduce func" % socket.gethostname())
   return a + b
...
def main(sc,filename,outfilename):
   ...
   while (curr_try_num < socket_retry_num and bSocketConnFail):
      ...
      try:
         ...
         util.logMessage("Job: %s: Other Unknown Error: %s!" % (APP_NAME, e))
         raise # not the error we are looking for
